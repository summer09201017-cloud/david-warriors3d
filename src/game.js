...
// —— 大衛的勇士們 3D(david-warriors3d,撒母耳記下 23 章)——2026-07-16,arena-duel-kit
// 聖經皮:fork warrior3d(徒步競技場對決引擎),引擎零改動、只換皮+經文。
// 玩家=大衛的勇士(以利亞撒/沙瑪/比拿雅/亞比篩,各配成名武器);對手=非利士勇士。
// 戰場=沙瑪死守的紅豆田(撒下 23:11-12);「三百人之戰」模式=亞比篩舉槍殺三百(撒下 23:18)。
// ★神學語氣:得勝歸耶和華——「那日耶和華使以色列人大獲全勝」(撒下 23:10,cuv 已查驗)。
// ★兒童安全鐵則:鈍頭武器、無流血;被擊中=後仰苦臉;敗方=溫柔跪地演出(不受傷)。
// ★判定=畫面(鐵則4):出手當下幾何判定,傷害延到接觸瞬間結算。
...
export const GAME_MODES = {
  duel: {
    label: "對決",
    hp: 100,
    description: "在紅豆田迎戰非利士勇士——自由走位,打光對手血量就得勝!",
    goal: "打光對手血量(各 100)",
  },
  epic: {
    label: "三百人之戰",
    hp: 300,
    roundCap: 300,
    description: "「他舉槍殺了三百人,就在三個勇士裡得了名。」(撒下 23:18)——雙方 300 血馬拉松,戰滿三百回合以血量判定。",
    goal: "血量 300,像亞比篩一樣戰到底",
  },
  practice: {
    label: "練習場",
    hp: 100,
    passive: true,
    description: "非利士人只走位不出手——自由練步法與八般武器手感。",
    goal: "純練手感,不計勝負",
  },
};
...
export const WEAPONS = {
  lance: { label: "長槍", short: "長槍", reach: 2.6, dmg: 16, cd: 1.6, arc: 0.55, chargeBonus: 0.9, swing: "lunge", hint: "比拿雅奪來的埃及長槍(撒下 23:21),衝刺加成最大" },
  spear: { label: "長矛", short: "長矛", reach: 2.4, dmg: 12, cd: 1.1, arc: 0.65, chargeBonus: 0.5, swing: "lunge", hint: "亞比篩的成名武器(撒下 23:18),長距直刺" },
  greatblade: { label: "彎月大刀", short: "大刀", reach: 2.0, dmg: 15, cd: 1.5, arc: 1.6, swing: "spin", hint: "360° 迴旋橫掃,重擊" },
  sword: { label: "勇士之劍", short: "劍", reach: 1.6, dmg: 10, cd: 0.8, arc: 1.25, swing: "chop", hint: "以利亞撒手黏刀把的那口劍(撒下 23:10)" },
  saber: { label: "彎刀", short: "彎刀", reach: 1.5, dmg: 8, cd: 0.55, arc: 1.35, swing: "chop", hint: "180° 快劈連擊" },
  rapier: { label: "短劍", short: "短劍", reach: 1.8, dmg: 6, cd: 0.4, arc: 0.7, swing: "lunge", hint: "最快的點刺" },
  bow: { label: "弓箭", short: "弓箭", ranged: true, dmg: 9, cd: 1.5, projSpeed: 26, maxRange: 30, hint: "遠距狙擊(鈍頭箭)" },
  greenballs: { label: "甩石索", short: "甩石", ranged: true, dmg: 6, cd: 2.4, projSpeed: 16, maxRange: 20, stun: 1.1, volley: 2, hint: "大衛家傳絕活——兩顆連投,命中暈眩" },
};
...
const WAVE_COLORS = { chop: 0xfff3b0, spin: 0xff9a3d, lunge: 0x6fd8ff, bow: 0xffe14d, greenballs: 0xd8d0b8 };
...
// ---------- 可選勇士(撒母耳記下 23 章,經文 cuv 已查驗) ----------
// 玩家四選一(各配成名武器,選了自動帶起手武器,仍可 1-8 換);對手固定=非利士勇士。
export const CHARACTERS = {
  eleazar: {
    label: "以利亞撒(劍)", shirt: 0x9c4a2f, pants: 0x5a3226, weapon: "sword",
    verse: "他起來擊殺非利士人,直到手臂疲乏,手黏住刀把。(撒下 23:10)",
  },
  shammah: {
    label: "沙瑪(大刀)", shirt: 0x3f7a3f, pants: 0x2e4a2a, weapon: "greatblade",
    verse: "沙瑪卻站在那田間擊殺非利士人,救護了那田。(撒下 23:12)",
  },
  benaiah: {
    label: "比拿雅(長槍)", shirt: 0x3a6a8a, pants: 0x24425a, weapon: "lance",
    verse: "從埃及人手裡奪過槍來,用那槍將他殺死。(撒下 23:21)",
  },
  abishai: {
    label: "亞比篩(長矛)", shirt: 0x7a4db8, pants: 0x3d2a5a, weapon: "spear",
    verse: "他舉槍殺了三百人,就在三個勇士裡得了名。(撒下 23:18)",
  },
};

// 非利士勇士(對手):青銅冠盔+紅纓羽冠(非利士戰士冠飾意象)+深色戰袍
function makePhilistine(scale) {
  const person = makePerson({ shirt: 0x565b66, pants: 0x3a3e46, hair: 0x2b2119, scale });
  const bronzeMat = new THREE.MeshStandardMaterial({ color: 0xb08d3f, metalness: 0.6, roughness: 0.4 });
  const helm = new THREE.Mesh(new THREE.CylinderGeometry(0.27, 0.28, 0.2, 16), bronzeMat);
  helm.position.y = 2.24;
  person.rig.add(helm);
  const brim = new THREE.Mesh(new THREE.CylinderGeometry(0.3, 0.3, 0.04, 16), bronzeMat);
  brim.position.y = 2.15;
  person.rig.add(brim);
  const crestMat = new THREE.MeshStandardMaterial({ color: 0xc23b22, roughness: 0.85 });
  for (let i = -2; i <= 2; i += 1) {
    const feather = new THREE.Mesh(new THREE.BoxGeometry(0.045, 0.22, 0.09), crestMat);
    feather.position.set(i * 0.055, 2.42, -0.02 - Math.abs(i) * 0.015);
    person.rig.add(feather);
  }
  return person;
}
...
function heroUp(person, teamColor, knotColor, withBand = true) {
  ...
  { // 甩石索(手握一顆石+備用一顆;投出時另生成)
    const balls = new THREE.Group();
    const ballMat = new THREE.MeshStandardMaterial({ color: 0x8f8f84, metalness: 0.1, roughness: 0.85 });
...
export class WarriorGame {
  constructor({ canvas, touchRoot }) {
    ...
    this.outfitId = "crimson"; // 已由勇士人選取代(保留欄位不炸舊存檔)
    this.characterId = CHARACTERS[settings.character] ? settings.character : "eleazar";
    this.weaponId = WEAPONS[settings.weaponId] ? settings.weaponId : CHARACTERS[this.characterId].weapon;
  ...
  setupScene() {
    ...
    // 沙瑪死守的紅豆田(撒下 23:11-12)——開放田地,中間不設任何阻擋
    const field = new THREE.Mesh(new THREE.PlaneGeometry(ARENA_HALF * 2 + 6, ARENA_HALF * 2 + 6), new THREE.MeshStandardMaterial({ color: 0x8f8a4e, roughness: 1 }));
    field.rotation.x = -Math.PI / 2;
    this.scene.add(field);
    // 紅豆叢地紋(平貼地面,純裝飾不擋路):一畦一畦的綠叢+紅豆點
    const bushMat = new THREE.MeshStandardMaterial({ color: 0x4a7a38, roughness: 1 });
    const beanMat = new THREE.MeshStandardMaterial({ color: 0xa8382e, roughness: 0.8 });
    for (let gx = -2; gx <= 2; gx += 1) {
      for (let gz = -2; gz <= 2; gz += 1) {
        if ((gx + gz) % 2 !== 0) continue; // 棋盤式田畦
        const cx = gx * 5.6;
        const cz = gz * 5.6;
        const patch = new THREE.Mesh(new THREE.CircleGeometry(1.5, 12), bushMat);
        patch.rotation.x = -Math.PI / 2;
        patch.position.set(cx, 0.002, cz);
        this.scene.add(patch);
        for (let b = 0; b < 5; b += 1) {
          const bean = new THREE.Mesh(new THREE.CircleGeometry(0.14, 8), beanMat);
          bean.rotation.x = -Math.PI / 2;
          bean.position.set(cx + Math.sin(b * 2.4) * 0.9, 0.004, cz + Math.cos(b * 2.4) * 0.9);
          this.scene.add(bean);
        }
      }
    }
  ...
  // 建(或重建)雙方——選勇士時整組重蓋(選單階段呼叫,不在戰鬥中換)
  _buildFighters() {
    ...
    const hero = CHARACTERS[this.characterId] || CHARACTERS.eleazar;
    this.my = this.makeFighter({ shirt: hero.shirt, pants: hero.pants, team: 0xb03030, knot: 0xf6d743 });
    this.foe = this.makeFighter({ team: 0x8a7a2a, knot: 0x3a3e46, philistine: true }); // 青銅盾
  ...
  setCharacter(charId) {
    ...
    this.weaponId = CHARACTERS[charId].weapon; // 勇士自帶成名武器(仍可 1-8 換)
    this._buildFighters();
    this.resetFighters();
  }

  makeFighter({ shirt, pants, team, knot, philistine = false }) {
    const person = philistine
      ? makePhilistine(1)
      : makePerson({ shirt, pants, scale: 1 });
    const gear = heroUp(person, team, knot, !philistine); // 非利士人戴青銅盔,不綁頭帶
  ...
  setOutfit() {
    // 戰袍配色已由「勇士人選」取代(各勇士有固定戰袍);保留空函式不炸舊呼叫
  }
  ...
  _fireProjectile(fighter, target, w) {
    ...
    if (fighter.weaponId === "greenballs") {
      ...
      mesh = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({ color: 0x8f8f84, metalness: 0.1, roughness: 0.85 }),
      );
    } else {
  ...
  finishMatch() {
    ...
    this.overlay = {
      visible: true,
      eyebrow: rWin ? "得勝!" : draw ? "平手" : "惜敗",
      title: byRounds ? `三百回合戰滿 ${this.my.hp}:${this.foe.hp}` : rWin ? "耶和華使以色列人大獲全勝!" : "這場讓非利士人拿下了",
      text: rWin
        ? `「那日耶和華使以色列人大獲全勝。」(撒母耳記下 23:10)——大戰 ${this.roundNo} 回合,勇士的得勝是耶和華賜的!`
        : draw
          ? "勢均力敵!換一把武器再來一場!"
          : `大戰 ${this.roundNo} 回合——像以利亞撒一樣再站起來,直到手黏住刀把!(撒下 23:10)`,
      canResume: false,
    };
