...
window.__davidwarriors3d = game; // dev hook
...
function handleGameEvent(event) {
  switch (event.type) {
    case "match-start": {
      ...
      audio.startCrowd();
      audio.vibrate(18);
      pushCommentary("大衛的勇士們——為以色列而戰!", "info", "大衛的勇士們,為以色列而戰!");
      break;
    }
    case "battle-start": {
      audio.horn();
    ...
    case "hit": {
      if (event.who === "me") {
        ...
        pushCommentary(
          ...
          "hot",
          event.stun ? "甩石命中,對手暈頭轉向!" : event.dmg >= 14 ? "正中要害,重重的一擊!" : "漂亮的一擊!",
        );
      } else {
        audio.thud(0.8);
        audio.vibrate(24);
    ...
    case "match-end": {
      try { if (!['localhost','127.0.0.1'].includes(location.hostname)) {   // -done:玩完一局(t=本局秒數,/stats 使用次數與平均停留吃這個)
        ...
        navigator.sendBeacon?.('https://hfpc-play-stats.summer09201017.workers.dev/api/ping?g=david-warriors3d-done&t=' + __dt);
      } } catch (_) {}
      pushCommentary(
        `終場!大戰 ${event.rounds} 回合,血量 ${event.myHp}:${event.aiHp}`,
        event.win ? "hot" : "info",
        event.win ? "勇士得勝,榮耀歸給耶和華!" : event.draw ? "平分秋色,再戰一場!" : "這場讓非利士人拿下了,再來!",
      );
      if (event.win) setTimeout(() => speakLine("那日耶和華使以色列人大獲全勝。"), 2200);
...
game.onHudUpdate = (state) => {
  ...
  ui.modeCode.textContent = ({ 對決: "對決", 三百人之戰: "三百人", 練習場: "練習" })[state.modeLabel] || state.modeLabel;
...
(function () {
  ...
  function _dwLeave() {
    ...
    if (s >= 3 && s <= 1800 && navigator.sendBeacon)
      navigator.sendBeacon("https://hfpc-play-stats.summer09201017.workers.dev/api/ping?g=david-warriors3d-dwell&t=" + s);
  }
